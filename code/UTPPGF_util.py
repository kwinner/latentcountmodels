...
def UTPPGF_compose(G, F, d='G'):
    # representation note: np.poly1d objects store coefficients in decreasing order
    ...
    Gbar = np.squeeze(G.data.copy())
    Gbar[0] = 0
    Fbar = np.squeeze(F.data.copy())
    Fbar[0] = 0

    H = np.polyval(Gbar[::-1],
                      np.poly1d(Fbar[::-1])) #F must be passed as a poly1d object or it is treated as a set of points to eval at

    # convert H back to a UTP
    H = np.array(H)[::-1]

    # reinsert the actual value of G (hopefully storing G(F(-))...)
    # H = np.insert(H, 0, G.data[0])
    H[0] = G.data[0]
