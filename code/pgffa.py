...
def evidence(a, b, f, y, rho):
    ...
    log_g = np.full((y + 1, len(f) + 1), -np.inf)
    df = f
    ...
    # g(s(1 - rho))
    h = np.arange(len(f), -1, -1) * np.log(1-rho)
    log_g = log_g + h

    g = np.exp(log_g)                         # scale back to non-log space
    g = np.poly1d(g) * np.poly1d([1] + [0]*y) # g * s**y
