...
inv.poiss <- function(mean.p) {
  return(c(lambda = as.double(mean.p)))
}
...
inv.nb <- function(mean.p, var.p) {
  ...
  return(c(r = as.double(r.q), p = as.double(p.q)))
}
...
inv.binom <- function(mean.p, var.p, y = NA) {
  ...
  n.q.exact <- as.double(mean.p / (1 - (var.p / mean.p)))

  # note: for a binomial, n.q must be integral
  n.q <- round(n.q.exact)
  if(mean.p > n.q)
    n.q <- ceiling(n.q.exact)
  
  # optionally lower bound n.q by y
  # note: this will always result in a distribution with the correct mean, but with increased variance
  # the resulting distribution will still always be underdispersed
  if (!is.na(y) && y > n.q) {
    # browser()
    n.q <- y
  }
  ...
  return(c(n = as.double(n.q), p = as.double(p.q)))
}
...
apgf <- function(F.pgf, logZ = NA, 
  ...
  if (is.nan(mean.p) || is.nan(var.p) || mean.p$sign < 0 || var.p$sign < 0)
    browser()
...
apgf2 <- function(F.pgf, logZ = NA, 
  ...
  # use the MGF to compute the first two moments of p
  # note: the second version of moments.p is done as much in logspace as possible
  # moments.p <- exp(F.mgf(lsgd.xdx(0, q = 3))$mag[2:3] + lgamma(2:3) - logZ$mag)
  moments.p <- as.ls(F.mgf(lsgd.xdx(0, q = 3))[2:3,]) * as.ls.magsign(mag = lgamma(2:3)) / logZ
  ...
  if (abs(mean.p - var.p) <= 0.1) {
    ...
    if (debug == TRUE)
      browser()
    distn    <- 'binom'
    theta.q  <- inv.binom(mean.p, var.p, y)
    if (theta.q['n'] < y)
      stop("Unable to approximate PGF with mean ", mean.p, " and var ", var.p)
    if (return.pgf)
      return.list$pgf <- function(s, theta = theta.q, logZ = logZ) { exp(logZ) * pgf.binom(s, theta) }
...
apgffwd <- function(y,
  ...
  for (i in 1:K) {
    ...
    if(is.na(y[i])) {
      Alpha.list[[i]] <- Gamma.list[[i]]
    } else {
      # approximate the Gamma message
      Gamma.logZ.list[[i]] <- Gamma.list[[i]](lsgd.1dx(q=1))[1,]
      hat.Gamma <- apgf2(Gamma.list[[i]], Gamma.logZ.list[[i]], return.distn = TRUE, return.param = TRUE, return.pgf = FALSE, debug = debug, y = y[i] + 1)
  ...
  val <- Alpha.list[[length(Alpha.list)]](lsgd.1dx(1), k = length(Alpha.list))
...
if(debug.apgffwd) {
  ...
  y      <- c(16, 17, 14, 15, 15)
  lambda <- data.frame(lambda = c(15.44563, 2.45214, 2.45214, 2.45214, 2.45214))
  delta  <- data.frame(lambda = c(51.70541, 51.70541, 51.70541, 51.70541, 51.70541),
                       p      = c(0.999999, 0.999999, 0.999999, 0.999999, 0.999999))
  rho    <- data.frame(p      = c(0.999999, 0.999999, 0.999999, 0.999999, 0.999999)) - 0.000099
  
  pgf.mixed_offspring <- function(s, theta) {
    return(pgf.poisson(s, theta) * pgf.bernoulli(s, theta))
  }
  
  print(apgffwd(y, pgf.poisson, lambda, pgf.mixed_offspring, delta, rho))
