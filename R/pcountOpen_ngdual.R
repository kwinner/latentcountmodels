source("ngdualforward.R")

pcountOpen_ngdual <- function(lambdaformula, gammaformula, omegaformula, pformula,
...
nll <- function(parms) {
    ...
    theta.arrival <- c(lambda, array(iota, T - 1))
    theta.offspring <- array(omega, T - 1)
    theta.observ <- array(det, T)
    
    ym <- array(c(31,12,16,22,15), c(1,5))
    
    # browser()
    
    Alpha <- ngdualforward(ym, arrival.pgf, theta.arrival, offspring.pgf, theta.offspring, theta.observ, d=1)
    
    # browser()
    
    return(-ngdualforward.ll(Alpha[[length(Alpha)]]))
...
environment(pcountOpen_ngdual) <- asNamespace('unmarked')
