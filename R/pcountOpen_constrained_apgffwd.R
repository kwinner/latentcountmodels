...
pcountOpen_apgffwd <- function(lambdaformula, gammaformula, omegaformula, pformula,
  ...
  if(mixture == "NB" || mixture == "ZIP" || dynamics == "notrend")
    can.rgdual = FALSE
  # these we likely cannot support
  if(dynamics == "ricker" || dynamics == "gompertz")
    can.rgdual = FALSE
  
  if(identical(nll.fun, 'rgd') && !can.rgdual)
  ...
  if(identical(nll.fun, "rgd") && !identical(method, "L-BFGS-B")) {
    warning("pcountOpen_apgffwd uses weak parameter bounds and L-BFGS-B for optimization, other method choices are ignored.")
  }
  ...
  if(identical(nll.fun, "trunc")) {
    ...
    objective.fun <- function(parms, debug=FALSE) {
      ...
      #TODO: notrend model, mixture models
  ...
  if(method == "L-BFGS-B") {
    ...
    if((dynamics == "ricker" || dynamics == "gompertz") && D$nOP > 0) {
      lower[(D$nAP+D$nGP+1):(D$nAP+D$nGP+D$nOP)] <- 1e-6
      upper[(D$nAP+D$nGP+1):(D$nAP+D$nGP+D$nOP)] <- 1 - (1e-6)
    }

    # detection param in 0 to 1
    lower[(D$nAP+D$nGP+D$nOP+1):(D$nAP+D$nGP+D$nOP+D$nDP)] <- 1e-6
    upper[(D$nAP+D$nGP+D$nOP+1):(D$nAP+D$nGP+D$nOP+D$nDP)] <- 1 - (1e-6)
  }
  
  # note: new default start option that tries to be a little smarter
  if(missing(starts) || is.na(starts))
    starts <- rep(0, D$nP)
  else if(is.numeric(starts) && length(starts) == 1)
    starts <- rep(starts, D$nP)
  else if(identical(starts, "auto") && identical(method, "L-BFGS-B")) {
    starts <- (upper - lower) / 2
  }
  
  # browser()
  # if (D$nAP > 0)
  #   starts[1:D$nAP] <- log(max(y[,1]))
  ...
  while(i.attempt <= n.attempts && is.null(fm)) {
    tryCatch({
      ...
      if(method == "L-BFGS-B")
        fm <- optim(starts, objective.fun, method=method, hessian=se, control = list(trace = TRUE), lower = lower, upper = upper, ...)
      else
        fm <- optim(starts, objective.fun, method=method, hessian=se, control = list(trace = TRUE), ...)
    }, error = function(e) {
      print(paste0('Attempt #', i.attempt, ' failed. Adding noise to initial values.'))
      fm <- NULL
    })
    starts <- starts + rexp(D$nP, 1e-4)
