...
t_vector <- cumsum(rep(t[3] / nrow(samples[[1]]), nrow(samples[[1]])))

samples_df <- data.frame(samples[[1]], iter = 1:nrow(samples[[1]]), time = t_vector)
...
mse_df <- data.frame(mse, iter = seq_along(mse))
ggplot(mse_df, aes(iter, mse)) +
  geom_line() +
  theme_bw() +
  ggtitle('MSE vs iteration') +
  ggsave(paste0('../plots/mse_vs_iter_', var, '.png'))
...
for (k in 1:K) {
  ...
  dist_plt_title <- bquote("Proportion of sampled values of" ~ n[.(k)])
  ggplot(samples_nk, aes(value, ..density..)) +
