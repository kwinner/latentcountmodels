...
def APGF(F_p, logZ = None, GDualType = gd.LSGDual, return_type = ['name', 'param', 'lambda']):
    ...
    mean_p = np.exp(log_moments_p[0])
    var_p  = np.exp(log_moments_p[1]) - (mean_p ** 2)
...
def APGF_Forward_symb(y,
    ...
    Alpha_lmbda_vec[0] = lambda s: GDualType.const(1.0)

    for i in range(K):
        # compute the Gamma message given Alpha[i] = Alpha_{i-1}(s_{i-1})
        if i == 0:
            Gamma = lambda u, k=i: GDualType.const(1.0) * immigration_pgf(u, theta_immigration[k])
        else:
            Gamma = lambda u, k=i: Alpha_lmbda_vec[k-1](offspring_pgf(u, theta_offspring[k-1])) * immigration_pgf(u, theta_immigration[k])
        logZ = Gamma(GDualType.const(1.0)).get(0, as_log = True)

        # compute Gamma_hat, the approximating distribution, using APGF
        Gamma_hat = APGF(Gamma, logZ, GDualType = GDualType, return_type = ['name', 'param'])
        ...
        if Gamma_hat_distn == 'poiss':
            Alpha_lmbda_vec[i] = lambda s_k, k=i, lmbda = Gamma_hat_theta[0]: \
                GDualType.const(y[k] * (np.log(lmbda) + np.log(rho[k])) - gammaln(y[k]) - lmbda + logZ, as_log=True) * \
                s_k ** y[k] * np.exp(lmbda * s_k * (1 - rho[k]))
        # elif Gamma_hat_distn == ''
